...
const DEMOS = [
	...
	},
	{
		...
		badges: ["=A1+B1", "=SUM(C1:C3)", "formula bar", "reactive updates", "reference highlighting"],
		tall: false,
	},
	{
		id: "clipboard",
	...
	},
	{
		...
		badges: ["fill handle", "linear series", "copy mode", "multi-column", "Escape cancel"],
		tall: false,
	},
	{
		id: "history",
	...
	},
	{
		...
		badges: ["per-column flag", "blocks edit", "blocks delete", "blocks paste"],
		tall: false,
	},
	{
		id: "large",
		...
		badges: ["10K rows", "200K cells", "virtual scrolling", "full edit support"],
		tall: true,
	},
	{
		id: "rows",
	...
	},
	{
		...
		badges: ["header select", "A-Z / Z-A", "visual only", "data unchanged"],
		tall: false,
	},
	{
		id: "sort-mutation",
		...
		badges: ["header select", "A-Z / Z-A", "reorders data", "row indices update"],
		tall: false,
	},
	{
		id: "sort-external",
		...
		badges: ["sort indicators", "onSort callback", "host-controlled", "server-side friendly"],
		tall: false,
	},
	{
		...
		badges: ["mutation sort", "=A1+B1", "formula bar", "refs shift on sort"],
		tall: false,
	},
	{
		id: "formula-rows",
		...
		badges: ["=B1+C1", "=SUM(D1:D3)", "insert row", "refs auto-shift", "formula bar"],
		tall: false,
	},
	{
		...
		badges: ["delete row", "=B3 ref shift", "=SUM range shrink", "dangling ref \u2192 error"],
		tall: false,
	},
	{
		id: "cross-sheet",
		...
		badges: ["=SUM(Data!B1:B3)", "=Data!A1", "workbook coordinator", "live sync"],
		tall: false,
	},
	{
		id: "custom-rendering",
	...
	},
	{
		id: "styling",
		tab: "Styling",
		title: "Theme + Row Styling",
		desc: "A host-owned midnight-ledger theme set through root CSS variables, plus focused-row styling from customization.getRowClass. Declarative range styles still layer through createRangeStyles.",
		badges: ["createRangeStyles", "getCellStyle", "getRowClass", "CSS variables", "focused row"],
		tall: false,
	},
] as const;
...
const GROUPS = [
	...
	{
		name: "Sorting",
		ids: ["sort-view", "sort-mutation", "sort-external", "sort-mutation-formulas"],
	},
...
const DEMO_BY_ID = new Map<DemoId, (typeof DEMOS)[number]>(DEMOS.map((d) => [d.id, d]));
		...
		<section class="hero-section">
			<div class="section-wrap">
				<div class="hero-shell rise-in">
					...
					<h1 class="hero-title">
						A spreadsheet engine <span class="hero-title-accent">built on signals.</span>
					</h1>
					<p class="hero-subtitle">
						Full spreadsheet UX for SolidJS — editing, formulas, clipboard, autofill, undo/redo —
						powered by fine-grained reactivity and built to stay performant.
					</p>
					<InstallCommand />
...
function DemoPlayground() {
	...
	const [viewMode, setViewMode] = createSignal<ViewMode>(hasScenarios() ? "replay" : "live");

	const group = () => GROUPS.find((g) => g.name === activeGroup()) ?? GROUPS[0];
	const demo = () => DEMO_BY_ID.get(activeId()) ?? DEMOS[0];

	// When the user picks a different demo-update three things atomically:
						...
						<div class={`demo-sheet-wrap${demo().tall ? " tall" : ""}`}>
							<div class={`demo-sheet-inner${demo().tall ? " tall" : ""}`}>
								<Show
									when={viewMode() === "code"}
									fallback={
										<div class={["demo-sheet-stage", { "replay-mode": viewMode() === "replay" }]}>
								...
								>
									<div class="demo-code-view">
										<pre>
											<code class="sh" innerHTML={highlight(DEMO_CODE[activeId()])} />
										</pre>
									</div>
								</Show>
							</div>
